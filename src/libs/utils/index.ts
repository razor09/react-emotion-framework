import { JsonArray, JsonObject, JsonPrimitive, JsonValue, Nullable, StringOrNumber } from './typings'
...
export namespace Utils {
  ...
  export const parse = <T extends JsonValue>(value: Nullable<string>): T => {
    if (value) {
      try {
  ...
  export const isEqual = (left: JsonValue, right: JsonValue): boolean => {
    if (isJsonObject(left) && isJsonObject(right)) {
      ...
      return isEqual(left.length, right.length) && left.every((value, index) => isEqual(value, right[index]))
    } else {
      return left === right
    }
  }
  ...
  export const debounce = (delay?: number): Function => {
    return (method: Function): Function => {
      let id: number
      return function (this: Function, ...args: JsonArray): void {
        self.clearTimeout(id)
        id = self.setTimeout(() => method.apply(this, args), delay)
      }
    }
  }
}
