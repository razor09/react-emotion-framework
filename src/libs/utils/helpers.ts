import { Nullable, Optional, PlainObject, PlainObjectOrArray, Primitive, StringOrBoolean, StringOrNumber } from './typings'
...
export namespace Helpers {
  ...
  export const shake = <T extends PlainObject<Optional<Primitive>>>(object: T, ignore: Optional<Primitive>[]): T => {
  ...
  export const equals = (left: unknown, right: unknown): boolean => {
    if ((isPrimitive(left) || isUndefined(left)) && (isPrimitive(right) || isUndefined(right))) {
      return left === right
    }
